...
=======
    this.isEditing    = true;
>>>>>>> 0ec953e445ab7863a94bcffbc4885075e0f7a60e
...
=======
    if (!confirm(`Voulez-vous vraiment supprimer "${monnaie.nom}" ?`)) return;
    this.monnaiesService.deleteMonnaie(monnaie.wikidataId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
          ...
          this.appliquerFiltres();
          this.fermerDetail();
          alert('🗑️ Monnaie supprimée avec succès.');
        },
        error: (err) => {
          if (err.status === 404) {
            this.toutesMonnaies = this.toutesMonnaies.filter(m => m.wikidataId !== monnaie.wikidataId);
...
>>>>>>> 0ec953e445ab7863a94bcffbc4885075e0f7a60e
  chargerMockData(): void {
    this.toutesMonnaies = [
      { wikidataId:'TUN001', nom:'Statère d\'Or de Carthage',       description:"Monnaie en or frappée à Carthage au IVe siècle av. J.-C. Symbole du pouvoir punique.", image:'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6e/BMC_Carthage_4.jpg/320px-BMC_Carthage_4.jpg', imageRevers:null, periode:'Punique',    materiau:'Or',     region:'Carthage',            atelier:'Carthage',     annee:-350, diametre:19.0, poids:9.4,  avers:'Tête de Tanit couronnée de laurier',  revers:'Cheval debout, palmier dattier',       collection:'Musée de Carthage' },
      ...
      { wikidataId:'TUN003', nom:'Bronze Punique de Carthage',      description:"Monnaie de bronze punique en circulation dans tout le territoire de Carthage.",          image:null, imageRevers:null, periode:'Punique',    materiau:'Bronze', region:'Carthage',            atelier:'Carthage',     annee:-300, diametre:20.0, poids:8.5,  avers:'Tête de Tanit à gauche',              revers:'Cheval au pas, légende punique',       collection:'Musée National de Carthage' },
      { wikidataId:'TUN004', nom:'Trishekel de Carthage',           description:"Grande monnaie d\'argent de Carthage valant 3 shekels.",                                 image:null, imageRevers:null, periode:'Punique',    materiau:'Argent', region:'Carthage',            atelier:'Carthage',     annee:-320, diametre:32.0, poids:21.5, avers:'Tête de Perséphone / Tanit',          revers:'Quadrige au galop conduit par aurige', collection:'Musée du Bardo, Tunis' },
